...
plot6.3 <- ggplot(cars)+geom_point(aes(x=speed,y=dist)) +
  scale_x_reverse(limits=c(30,0),expand=c(0,0))+
  scale_y_continuous("cars$dist",limits=c(0,150),expand=c(0,0))+
  theme_bw() +
  opts(axis.title.x = theme_text(vjust=-0.25))

recipe6c <-function() {
doComboPlot(doBasePlot=base.recipe.6c,gplot=plot6.3)
}
